...
import com.abdoali.datasourece.api.parserReadsMp3
...
class ApiQuran @Inject constructor(private val apiQuran: ApiService) {
    ...
    suspend fun getAllMp3quran(): List<Quran> {
        return try {

            val jsonObject = JSONObject(apiQuran.getListReciters())
            parserReciterMp3(jsonObject).asQuranList()
        } catch (e: Exception) {
            Log.i("Exception" , e.message.toString())
            emptyList<Quran>()
        }
    ...
    suspend fun getFovMp3quran(): List<Quran> {
        val list = getAllMp3quran().shuffled()
        var index = 0
